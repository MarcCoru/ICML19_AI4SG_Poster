...
\begin{minipage}[t]{.65\textwidth}
	...
	\node[label={[name=cm,text height=1.5ex,text depth=.25ex]{Early} {Classification} Model}, anchor=north](f) at (0,0){ $\yhat, {\color{tumbluedark}\deltat} = f(\xuptot)$};
%	\node[below=of f](example){\input{images/example.tikz}};
	\node[circle, below=of f, text width=11cm, fill=tumbluedark, text=white](example){Classifying a satellite time series {\color{tumbluelight}\textbf{accurately}} \color{white} as {\color{ecolor}\textbf{early}} \color{white} as possible};
